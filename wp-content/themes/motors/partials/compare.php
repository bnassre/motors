...
<?php else: ?>
	...
	<?php
				...
				if($compares->have_posts()): ?>
					<div class="row row-4 car-listing-row stm-car-compare-row">
						...
						<?php while($compares->have_posts()): $compares->the_post(); $counter++; ?>
							...
							<div class="col-md-3 col-sm-3 compare-col-stm compare-col-stm-<?php echo esc_attr(get_the_ID()); ?>">
									...
									<div class="compare-col-stm-empty">
										<div class="image">
											...
											<?php } else { ?>
												...
												<img class="stm-compare-empty" src="<?php echo esc_url(get_stylesheet_directory_uri().'/assets/images/compare-empty.jpg'); ?>" alt="<?php esc_html_e('Empty', 'motors'); ?>"/>
											<?php }; ?>
										</div>
									</div>
								</a>
										...
										<div class="car-meta-top heading-font clearfix">
											...
											<?php if(!empty($price) and !empty($sale_price)):?>
												<div class="price discounted-price">
													<div class="regular-price"><?php echo esc_attr(stm_get_price_currency().number_format($price, 0, '', ' ')); ?></div>
													<div class="sale-price"><?php echo esc_attr(stm_get_price_currency().number_format($sale_price, 0, '', ' ')); ?></div>
												</div>
											<?php elseif(!empty($price)): ?>
												<div class="price">
													<div class="normal-price"><?php echo esc_attr(stm_get_price_currency().number_format($price, 0, '', ' ')); ?></div>
												</div>
											<?php endif; ?>
										...
										<?php foreach($filter_options as $filter_option): ?>
											<?php if($filter_option['slug'] != 'price'){ ?>
												<tr>
													...
													<td class="compare-value-hover" data-value="<?php echo esc_attr('compare-value-'.$filter_option['slug']) ?>">
			...
			<?php if(!empty($compares)): ?>
				<?php if($compares->have_posts()): ?>
					<div class="row row-4 row-compare-features">
						<div class="col-md-3 col-sm-3">
							<h4 class="stm-compare-features"><?php echo esc_attr('Additional features', 'motors'); ?></h4>
						</div>
						<?php while($compares->have_posts()): $compares->the_post(); ?>
		...
		<div class="compare-empty-car-top">
			<div class="col-md-3 col-sm-3 compare-col-stm-empty">
				<a href="<?php echo esc_url( get_post_type_archive_link( 'listings' ) ); ?>">
					<div class="image">
						<i class="stm-icon-add_car"></i>
						<img class="stm-compare-empty" src="<?php echo esc_url( get_stylesheet_directory_uri() . '/assets/images/compare-empty.jpg' ); ?>"  alt="<?php esc_html_e('Empty', 'motors'); ?>"/>
					</div>
