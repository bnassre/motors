					...
					<?php if ( $counter == 1 and empty( $modern_filter_unit['use_on_car_modern_filter_view_images'] ) and ! $modern_filter_unit['use_on_car_modern_filter_view_images'] ): ?>
						<?php if ( ! empty( $terms ) ): ?>
							<div class="stm-accordion-single-unit <?php echo esc_attr($modern_filter_unit['slug']); ?>">
								<a class="title" data-toggle="collapse" href="#<?php echo esc_attr($modern_filter_unit['slug']) ?>" aria-expanded="true">
										...
										<div class="collapse in content" id="<?php echo esc_attr($modern_filter_unit['slug']); ?>">
											<div class="stm-accordion-content-wrapper">
												<div class="stm-single-unit-wrapper">
													...
													<?php $images = 0; foreach ( $terms as $term ): $images ++; ?>
														...
														if(!empty($image)): $number_of_images++; ?>
															<div class="stm-single-unit-image">
																<label>
																	<?php if ( ! empty( $category_image ) ): ?>
																		<span class="image">
																			<img class="img-reponsive" src="<?php echo esc_url( $category_image ); ?>" alt="<?php esc_html_e('Brand', 'motors'); ?>"/>
																		</span>
																	<?php endif; ?>
																	<input type="checkbox"
																	       name="<?php echo esc_attr( $term->slug . '-' . $term->term_id ); ?>"
													...
													<?php if($number_of_images < count($terms)): ?>
														...
														<div class="stm-modern-filter-others">
															<?php $non_images = 0; foreach ( $terms as $term ): $non_images ++; ?>
	...
	<div class="col-md-9 col-sm-12 <?php echo esc_attr($content_pos_classes); ?>">
		<div class="stm-car-listing-sort-units stm-modern-filter-actions clearfix">
			<div class="stm-modern-filter-found-cars">
				<h4><span class="orange"><?php echo esc_attr($listings->found_posts); ?></span> <?php esc_html_e('Vehicles available', 'motors'); ?></h4>
			</div>
			<?php
			$view_list = '';
			$view_grid = '';
			...
			<div class="stm-view-by">
				<a href="?view_type=grid" class="view-grid view-type <?php echo esc_attr($view_grid); ?>">
					<i class="stm-icon-grid"></i>
				</a>
				<a href="?view_type=list" class="view-list view-type <?php echo esc_attr($view_list); ?>">
			...
			<div class="stm-sort-by-options clearfix">
				...
				<div class="stm-select-sorting">
					<select>
						<option value="date_high" selected><?php echo esc_attr('Date: newest first'); ?></option>
						<option value="date_low"><?php echo esc_attr('Date: oldest first'); ?></option>
						<option value="price_low"><?php echo esc_attr('Price: lower first'); ?></option>
						<option value="price_high"><?php echo esc_attr('Price: highest first'); ?></option>
						<option value="mileage_low"><?php echo esc_attr('Mileage: lowest first'); ?></option>
						<option value="mileage_high"><?php echo esc_attr('Mileage: highest first'); ?></option>
					</select>
				</div>
			</div>
		</div>
